...
//設定跳回的應用程式網址
            ...
            $redirectUrl = 'http://localhost/Fb_questionnaire/intro/';
//設定跳回的應用程式網址
...
$app->get('/intro/', function () use($app) {
            ...
            $app_id = '508173905905976';
            $secret = 'cf27054f46cc2e3bd45d2422046f351b';
            $redirectUrl = 'http://localhost/Fb_questionnaire/intro/';//'/Fb_questionnaire.freeoda.com';
            ...
            /* $dsn = "mysql:host=localhost;dbname=Fb_questionnaire";
              $db = new PDO($dsn, 'root','');
...
$app->get("/login&:status", function ($status) use ($app) {
            ...
            if (isset($_SESSION['urlRedirect'])) {
                $urlRedirect = '/Fb_questionnaire' . $_SESSION['urlRedirect'];
            }
...
function chart2($query) {
    ...
    return $http . '&amp;cht=bvs&amp;chd=' . $chd .
            '&amp;chdl=Male|Female&amp;chs=400x300&amp;chco=0081ff,d41243&amp;chxt=x,y&amp;chxl=' . $chl .
